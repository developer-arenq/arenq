...
const Art = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch("/api/products/featured/best_sellers", {
          cache: "no-store",
        });
  ...
  const scrollBy = (dir) => {
    ...
    const amt = scrollRef.current.offsetWidth * 0.9;
      ...
      <div className="relative mx-auto w-[100%] lg:w-[95%] max-w-[1400px] px-2 py-6">
        ...
        <div className="w-full flex items-center justify-between gap-2">
          ...
          <div className="flex-1 flex justify-start">
            <h2 className="text-[18px] sm:text-[28px] md:text-[36px] lg:text-[40px] font-bold text-[#2d241b] whitespace-nowrap leading-tight tracking-wide">
              Art & Crafts
            </h2>
          </div>
          ...
          <div className="flex-1 flex justify-end">
            <Link
              href="/search?category=Handcrafts"
              className="px-3 py-1 sm:px-4 sm:py-2 bg-[#2d241b] text-white rounded-lg 
              shadow hover:bg-[#524232] transition text-xs sm:text-sm whitespace-nowrap"
            >
              View All
            </Link>
          </div>
        ...
        <button
          ...
          className="hidden sm:flex absolute top-1/2 -translate-y-1/2 -left-10
                     z-20 bg-white border border-gray-300 text-gray-700 rounded-full p-3 
          ...
          {loading
            ...
            : products.map((p, i) => {
              ...
              return (
                <div
                  key={p._id + i}
                  className="cursor-pointer relative w-[48%] sm:w-[260px] lg:w-[285px]
                    flex-shrink-0 border border-green-400 hover:border-green-700 
                    bg-white rounded-xl overflow-hidden group snap-start shadow-sm 
               ...
               <Link href={`/products/${p.slug}`}>
                    <div className="relative overflow-hidden flex flex-col h-full">
                      ...
                      {out && (
                        <span className="absolute top-2 left-2 z-20 bg-red-600 text-white 
                          text-[10px] sm:text-xs font-bold px-2 py-1 rounded">
                          Out of Stock
                        </span>
                      )}
...
"
                      ...
                      {/* Buy Now */}
                      <div className="absolute bottom-0 left-0 w-full bg-white/95 backdrop-blur-sm
                        sm:translate-y-full sm:group-hover:translate-y-0 transition-transform duration-200 z-10">
                        <button
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
        ...
        <button
          ...
          className="hidden sm:flex absolute top-1/2 -translate-y-1/2 -right-10
                     z-20 bg-white border border-gray-300 text-gray-700 rounded-full p-3 
                     shadow-md hover:bg-gray-200 transition"
        >
