...
import { initialCart } from "../slices/cart"; // make sure you have this import
...
const OfferProduct = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/products/featured/top_deals`);
        if (!res.ok) throw new Error("Failed to fetch");
        const data = await res.json();
        setProducts(data.slice(0, 8)); // Optional: limit to 8 products
      } catch (error) {
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={(e) => !isOutOfStock && wishlistHandler(product._id, e)}
                    ...
                    <div className="cursor-pointer relative">
                      ...
                      <div className="relative overflow-hidden rounded-md">
                        <Image
                          ...
                          alt={product?.alt_text || product?.name || "Product Image"}
                          width={600}
                          height={600}
                          objectFit="cover"
                          priority={index < 8}
